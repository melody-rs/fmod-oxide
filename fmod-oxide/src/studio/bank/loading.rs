...
impl Bank {
    ...
    pub fn get_loading_state(&self) -> Result<LoadingState> {
        ...
        let error = unsafe {
            FMOD_Studio_Bank_GetLoadingState(self.as_ptr(), &raw mut loading_state).to_error()
        };
    ...
    pub fn load_sample_data(&self) -> Result<()> {
        unsafe { FMOD_Studio_Bank_LoadSampleData(self.as_ptr()).to_result() }
    }
    ...
    pub fn unload_sample_data(&self) -> Result<()> {
        unsafe { FMOD_Studio_Bank_UnloadSampleData(self.as_ptr()).to_result() }
    }
    ...
    pub fn get_sample_loading_state(&self) -> Result<LoadingState> {
        let mut loading_state = 0;
        let error = unsafe {
            FMOD_Studio_Bank_GetSampleLoadingState(self.as_ptr(), &raw mut loading_state)
                .to_error()
        };
        LoadingState::try_from_ffi(loading_state, error)
    }
    ...
    pub fn unload(self) -> Result<()> {
        ...
        unsafe { FMOD_Studio_Bank_Unload(self.as_ptr()).to_result() }
    }
}
