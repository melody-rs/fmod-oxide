...
impl EventInstance {
    ...
    pub fn get_channel_group(&self) -> Result<ChannelGroup> {
        ...
        unsafe {
            FMOD_Studio_EventInstance_GetChannelGroup(self.as_ptr(), &raw mut channel_group)
                .to_result()?;
    ...
    pub fn set_reverb_level(&self, index: c_int, level: c_float) -> Result<()> {
        unsafe {
            FMOD_Studio_EventInstance_SetReverbLevel(self.as_ptr(), index, level).to_result()
        }
    }
    ...
    pub fn get_reverb_level(&self, index: c_int) -> Result<c_float> {
        let mut level = 0.0;
        unsafe {
            FMOD_Studio_EventInstance_GetReverbLevel(self.as_ptr(), index, &raw mut level)
                .to_result()?;
        }
        Ok(level)
    }
