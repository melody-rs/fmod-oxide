...
impl System {
    ...
    pub fn set_output(&self, output_type: OutputType) -> Result<()> {
        unsafe { FMOD_System_SetOutput(self.as_ptr(), output_type.into()).to_result() }
    }
    ...
    pub fn get_output_type(&self) -> Result<OutputType> {
        ...
        unsafe {
            FMOD_System_GetOutput(self.as_ptr(), &raw mut output_type).to_result()?;
        }
    ...
    pub fn get_driver_count(&self) -> Result<c_int> {
        let mut count = 0;
        unsafe {
            FMOD_System_GetNumDrivers(self.as_ptr(), &raw mut count).to_result()?;
        }
        Ok(count)
    }
    ...
    pub fn set_driver(&self, driver: c_int) -> Result<()> {
        unsafe { FMOD_System_SetDriver(self.as_ptr(), driver).to_result() }
    }
    ...
    pub fn get_driver(&self) -> Result<c_int> {
        let mut driver = 0;
        unsafe {
            FMOD_System_GetDriver(self.as_ptr(), &raw mut driver).to_result()?;
        }
        Ok(driver)
    }
}
