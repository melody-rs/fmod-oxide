...
impl System {
    ...
    pub fn lock_dsp(&self) -> Result<DspLockGuard> {
        unsafe { FMOD_System_LockDSP(self.as_ptr()).to_result()? };
    ...
    pub fn set_userdata(&self, userdata: *mut c_void) -> Result<()> {
        unsafe { FMOD_System_SetUserData(self.as_ptr(), userdata).to_result() }
    }
    ...
    pub fn get_userdata(&self) -> Result<*mut c_void> {
        ...
        unsafe {
            FMOD_System_GetUserData(self.as_ptr(), &raw mut userdata).to_result()?;
        }
        Ok(userdata)
    }
}
