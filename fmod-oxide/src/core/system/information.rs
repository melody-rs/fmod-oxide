...
impl System {
    ...
    pub fn get_version(&self) -> Result<(c_uint, c_uint)> {
        ...
        unsafe {
            FMOD_System_GetVersion(self.as_ptr(), &raw mut version, &raw mut build_number)
                .to_result()?;
        }
    ...
    pub fn get_version(&self) -> Result<c_uint> {
        let mut version = 0;
        unsafe {
            FMOD_System_GetVersion(self.as_ptr(), &mut version).to_result()?;
        }
        Ok(version)
    }
    ...
    pub fn get_output_handle(&self) -> Result<*mut c_void> {
        ...
        unsafe {
            FMOD_System_GetOutputHandle(self.as_ptr(), &raw mut handle).to_result()?;
        }
        Ok(handle)
    }
    ...
    pub fn get_cpu_usage(&self) -> Result<CpuUsage> {
        ...
        unsafe {
            FMOD_System_GetCPUUsage(self.as_ptr(), cpu_usage.as_mut_ptr()).to_result()?;
