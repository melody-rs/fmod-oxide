...
impl ChannelControl {
    ...
    pub fn get_audibility(&self) -> Result<c_float> {
        ...
        unsafe {
            FMOD_ChannelControl_GetAudibility(self.as_ptr(), &raw mut audibility)
                .to_result()?;
        }
        Ok(audibility)
    }
    ...
    pub fn set_volume(&self, volume: c_float) -> Result<()> {
        unsafe { FMOD_ChannelControl_SetVolume(self.as_ptr(), volume).to_result() }
    }
    ...
    pub fn get_volume(&self) -> Result<c_float> {
        let mut volume = 0.0;
        unsafe {
            FMOD_ChannelControl_GetVolume(self.as_ptr(), &raw mut volume).to_result()?;
        }
        Ok(volume)
    }
    ...
    pub fn set_volume_ramp(&self, ramp: bool) -> Result<()> {
        unsafe { FMOD_ChannelControl_SetVolumeRamp(self.as_ptr(), ramp).to_result() }
    }
    ...
    pub fn get_volume_ramp(&self) -> Result<bool> {
        let mut ramp = false;
        unsafe {
            FMOD_ChannelControl_GetVolumeRamp(self.as_ptr(), &raw mut ramp).to_result()?;
        }
        Ok(ramp)
    }
    ...
    pub fn set_mute(&self, mute: bool) -> Result<()> {
        unsafe { FMOD_ChannelControl_SetMute(self.as_ptr(), mute).to_result() }
    }
    ...
    pub fn get_mute(&self) -> Result<bool> {
        let mut mute = false;
        unsafe {
            FMOD_ChannelControl_GetMute(self.as_ptr(), &raw mut mute).to_result()?;
        }
        Ok(mute)
    }
}
